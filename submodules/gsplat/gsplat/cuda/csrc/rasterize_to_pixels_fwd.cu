...
__global__ void rasterize_to_pixels_fwd_kernel(
    ...
    int32_t *__restrict__ depth_idx, // [C, image_height, image_width]
    bool *__restrict__ is_used       // [C, N]
) {
    ...
    depth_idx += camera_id * image_height * image_width;
    ...
    uint32_t cur_idx = 0;
    // index of the gaussian for depth map calculation
    int32_t cur_depth_idx = -1;
    S max_vis = 0.0f;
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            int32_t g = id_batch[t]; // global index of this gaussian
            const S vis = alpha * T;
            ...
            // Record the index of the gaussian for depth map calculation.
            if (cur_depth_idx == -1 || T >= 0.7f) {
                cur_depth_idx = g;
            }
            // if (vis > max_vis) {
            //     max_vis = vis;
    ...
    if (inside) {
        ...
        depth_idx[pix_id] = cur_depth_idx;
    }
}
...
call_kernel_with_dim(
    ...
    torch::Tensor depth_idx = torch::empty(
        {C, image_height, image_width}, means2d.options().dtype(torch::kInt32)
    );
    torch::Tensor is_used = torch::full(
        {C, N}, false, means2d.options().dtype(torch::kBool)
    );
    ...
    rasterize_to_pixels_fwd_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            depth_idx.data_ptr<int32_t>(),
            is_used.data_ptr<bool>()
        );

    return std::make_tuple(renders, alphas, last_ids, depth_idx, is_used);
}
