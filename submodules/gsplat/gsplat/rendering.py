...
def rasterization(
    ...
    .. note::
        **Depth Rendering**: This function supports colors or/and depths via `render_mode`.
        The supported modes are "RGB", "D", "ED", "MD", "RGB+D", "RGB+ED", and "RGB+MD". "RGB" renders
        the colored image that respects the `colors` argument. "D" renders the accumulated z-depth
    ...
    Args:
        ...
        render_mode: The rendering mode. Supported modes are "RGB", "D", "ED", "MD", "RGB+D",
            "RGB+ED" and "RGB+MD". "RGB" renders the colored image, "D" renders the accumulated depth,
            "ED" renders the expected depth, and "MD" renders the median depth. Default is "RGB".
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            render_colors_, render_alphas_, depth_idx, is_used = rasterize_to_pixels(
                means2d,
                conics,
                colors_chunk,
                opacities,
        ...
        render_colors, render_alphas, depth_idx, is_used = rasterize_to_pixels(
            means2d,
            conics,
            colors,
            opacities,
    ...
    elif render_mode in ["MD", "RGB+MD"]:
        with torch.no_grad():
            # Get median depth by taking the maximum response

            # Normalized ray direction
            row_idx, col_idx = torch.meshgrid(torch.arange(height, device=device), torch.arange(width, device=device), indexing="ij")
            ppix = torch.stack((col_idx, row_idx), dim=-1) + 0.5
            ndc = ppix / torch.tensor([width, height], device=device, dtype=torch.float32) * 2.0 - 1.0
            ndc = ndc.reshape(1, -1, 2).expand(C, -1, -1)  # [C, height * width, 2]
            tan_fovx = (0.5 * width / Ks[:, 0, 0])         # [C]
            tan_fovy = (0.5 * height / Ks[:, 1, 1])        # [C]
            cam_dir_x = viewmats[:, 0, :3].double().reshape(C, 1, 3)  # right
            cam_dir_y = viewmats[:, 1, :3].double().reshape(C, 1, 3)  # down
            cam_dir_z = viewmats[:, 2, :3].double().reshape(C, 1, 3)  # forward

            # Normalized ray direction. [C * height * width, 3]
            ray_dir = cam_dir_z + ndc[..., 0:1] * tan_fovx * cam_dir_x + ndc[..., 1:2] * tan_fovy * cam_dir_y
            ray_dir = ray_dir.reshape(-1, 3).double()
            ray_dir = F.normalize(ray_dir, dim=-1)  # normalize
            
            # Max response
            intersect_xyzs = means[depth_idx.flatten(), :].double().reshape(-1, 3)  # [C * height * width, 3]
            intersect_scales = scales[depth_idx.flatten(), :].reshape(-1, 3)  # [C * height * width, 3]
            intersect_quats = quats[depth_idx.flatten(), :].reshape(-1, 4)  # [C * height * width, 4]
            cam_pos = torch.inverse(viewmats)[:, :3, 3] # [C, 3]

            # Reshape cam_pos. [C * height * width, 3]
            cam_pos = cam_pos.unsqueeze(1).expand(-1, height * width, -1).reshape(-1, 3)

            invS = torch.diag_embed(1.0 / intersect_scales.double())                    # [C * height * width, 3, 3]
            invR = normalized_quat_to_rotmat(intersect_quats).transpose(1, 2).double()  # [C * height * width, 3, 3]
            inv_RS = torch.bmm(invS, invR)

            dg = torch.bmm(inv_RS, ray_dir.unsqueeze(-1)).squeeze(-1)  # [C * height * width, 3]
            og = torch.bmm(inv_RS, (cam_pos - intersect_xyzs).unsqueeze(-1)).squeeze(-1) # [C * height * width, 3]

            tau = -torch.sum(dg * og, dim=-1) / torch.sum(dg * dg, dim=-1)  # [C * height * width]
            depth_map = tau * torch.sum(ray_dir.reshape(C, -1, 3) * cam_dir_z, dim=-1).flatten()
            
            # No intersection
            depth_map[depth_idx.flatten() == -1] = far_plane
            # Too close to the camera
            depth_map[depth_map < near_plane] = near_plane

            depth_map = depth_map.reshape(C, height, width).float()
        
        render_colors = depth_map[..., None] if render_mode == "MD" else torch.cat(
            [render_colors, depth_map[..., None]], dim=-1
        )
