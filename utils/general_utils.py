...
def safe_state(silent, seed=0):
    ...
    # Initialize the linalg module for multi-threading safety
...
# scale.shape == [N, 2]
# rotation.shape == [N, 1]
# return 2d covariance matrix. shape == [N, 3]
def compute_cov2D(scale, rotation):
    sin_theta = torch.sin(rotation) # [N, 1]
    cos_theta = torch.cos(rotation) # [N, 1]
    a2 = scale[:, 0:1] ** 2 # [N, 1]
    b2 = scale[:, 1:2] ** 2 # [N, 1]

    return torch.cat([
        a2 * cos_theta * cos_theta + b2 * sin_theta * sin_theta,
